...
function [y, dy, ddy] = roadProfile(x, road)
    xStraight = 5;     % first 5 m fully straight
    xBlendEnd = 10;    % finish blending by 10 m

    % shifted coordinate for the actual curved road
    xs = max(x - xStraight, 0);

    % ----- original curved road definition -----
    switch road.type
        case 1
            y_curve = 0;
            dy_curve = 0;
        ...
        case 2
            w = 2*pi/road.L;
            y_curve   = road.A * sin(w*xs);
            dy_curve  = road.A * w * cos(w*xs);
            ddy_curve = -road.A * w^2 * sin(w*xs);

        case 3
            w1 = 2*pi/road.L1;
            w2 = 2*pi/road.L2;
            y_curve   = road.A1*sin(w1*xs) + road.A2*sin(w2*xs);
            dy_curve  = road.A1*w1*cos(w1*xs) + road.A2*w2*cos(w2*xs);
            ddy_curve = -road.A1*w1^2*sin(w1*xs) - road.A2*w2^2*sin(w2*xs);

        case 4
            z1 = (xs-road.x1)/road.w;
            z2 = (xs-road.x2)/road.w;

            y_curve = road.A*tanh(z1) - road.A*tanh(z2);
            dy_curve = road.A*(sechLocal(z1)^2)/road.w ...
                     - road.A*(sechLocal(z2)^2)/road.w;
            ddy_curve = road.A*(-2*sechLocal(z1)^2*tanh(z1))/road.w^2 ...
    ...
    elseif x >= xBlendEnd
        ...
        t = (x - xStraight) / (xBlendEnd - xStraight);

        s = 3*t^2 - 2*t^3;
        ds = (6*t - 6*t^2) / (xBlendEnd - xStraight);
        dds = (6 - 12*t) / (xBlendEnd - xStraight)^2;
    end

    % ----- blended road -----
    y   = s * y_curve;
    dy  = ds * y_curve + s * dy_curve;
    ddy = dds * y_curve + 2*ds * dy_curve + s * ddy_curve;
end
